...
- Reusing worker-local regcmd/input/partials scratch BOs did not improve decode: `10.83 ± 1.23 t/s` versus an adjacent `11.21 ± 1.30 t/s` baseline. Do not reimplement this without new evidence.
...
- This result proves a scheduling/performance improvement relative to the current native-W8 implementation; it does **not** clear the project correctness gate. The current native-W8 sidecar path itself already produces the known bad continuation `The capital of France is Paris cityss France ...`, unlike the correctness reference used by production paths. Direct-submit preserves that existing output exactly rather than fixing it.
- Keep direct-submit on the native-W8 experimental line until the sidecar/quantization correctness issue is resolved. Do not merge it to production/main merely on the throughput result.
