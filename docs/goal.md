...
ONNX import, a future GGUF/LLM frontend, and Candle/framework integration should all lower into this same project-owned contract and use the same preparation, placement, residency, scheduling, CPU fallback, and Rocket backend machinery. The next internal cleanup is to move the validated CNN graph executor itself out of the ONNX adapter crate so the crate dependency graph mirrors the public architecture as cleanly as the API now does.
