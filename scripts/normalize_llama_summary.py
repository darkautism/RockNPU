...
def main():
    ...
    out={'mean_time_ns':{k:v['mean_time_ns'] for k,v in means.items()},'mean_tok_s':{k:v['mean_tok_s'] for k,v in means.items()},'speedup_from_mean_time':means['cpu']['mean_time_ns']/means['npu']['mean_time_ns'],'speedup_from_mean_tok_s':means['cpu']['mean_tok_s']/means['npu']['mean_tok_s'],'block_speedups':[]}
    for i in range(0,len(rows),4):
        b=rows[i:i+4]; c=[x['result'] for x in b if x['kind']=='cpu']; n=[x['result'] for x in b if x['kind']=='npu']; out['block_speedups'].append({'time':(sum(x['avg_ns'] for x in c)/len(c))/(sum(x['avg_ns'] for x in n)/len(n)),'tok_s':(sum(x['avg_ts'] for x in c)/len(c))/(sum(x['avg_ts'] for x in n)/len(n))})
