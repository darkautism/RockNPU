...
def env_for(args, port, kind):
    ...
    e.update({'OLLAMA_HOST':f'127.0.0.1:{port}','OLLAMA_MODELS':str(args.models),'OLLAMA_NUM_GPU':'0' if kind=='oracle' else '999','OLLAMA_KEEP_ALIVE':'30m','OLLAMA_VULKAN':'true','LLAMA_ARG_DEVICE':'none' if kind=='oracle' else ('Vulkan0' if kind=='gpu' else 'ROCKNPU0')})
    ld=str(args.lib_dir)
    if kind=='gpu':
