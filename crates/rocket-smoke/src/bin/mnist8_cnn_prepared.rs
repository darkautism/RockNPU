...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut prepared_conv = CnnPreparedConvState::new();
    ...
    for sample in 0..100 {
        ...
        let (out, stats, trace) = if sample == 0 {
            let (o, s, t) = model.run_fp16_traced_with_prepared_conv(
                &x,
                ExecutionTarget::NpuSingle,
                Some(&mut backend),
                Some(&mut conv_backend),
                Some(&mut prepared_conv),
            )?;
            (o, s, Some(t))
        } else {
            let (o, s) = model.run_fp16_with_prepared_conv(
                &x,
                ExecutionTarget::NpuSingle,
                Some(&mut backend),
                Some(&mut conv_backend),
        ...
        if sample == 0 {
            ...
            let ps = prepared_conv.stats();
            if ps.tensors != 2 || ps.resident_bytes != 51_200 {
                return Err(format!(
                    "unexpected prepared Conv state after first inference: {ps:?}"
                )
                .into());
            }
            let ds = prepared_conv.dense_stats();
            if ds.tensors != 1 || ds.resident_bytes == 0 {
                return Err(format!(
                    "unexpected prepared dense state after first inference: {ds:?}"
                )
                .into());
            }
            if conv_backend.scratch_stats().weight_bytes != 0 {
                return Err(format!(
                    "prepared Conv path owns weight scratch: {:?}",
        ...
        if let Some(trace) = trace {
            ...
            fs::write("artifacts/mnist8-prepared-trace.tsv", meta)?;
            fs::write("artifacts/mnist8-prepared-trace-f16.bin", raw)?;
            trace_saved = true;
        }
    }
    if pred_match != 100 {
    ...
    fs::write("artifacts/mnist8-prepared100-f16.bin", all_output_bits)?;
    let mean_abs = sum_abs as f32 / 1000.0;
    let stats = accepted_stats.unwrap();
    println!(
        "MNIST-8 CNN PREPARED-CONV PASS nodes={} conv_nodes={} conv_npu={} pool_cpu={} reshape_cpu={} add_cpu={} relu_cpu={} matmul_npu={} padded_npu={} top1_ref={}/100 ref_accuracy={}/100 npu_accuracy={}/100 max_abs={:.8} mean_abs={:.8} trace_saved={} conv_scratch={:?} prepared_conv={:?} prepared_dense={:?}",
        model.node_count(),
        stats.conv_nodes,
