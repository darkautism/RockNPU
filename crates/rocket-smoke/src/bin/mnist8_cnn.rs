...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for sample in 0..100 {
        ...
        let (out, stats, trace) = if sample == 0 {
            let (o, s, t) =
                model.run_fp16_traced(&x, ExecutionTarget::NpuSingle, Some(&mut backend))?;
            (o, s, Some(t))
        } else {
            let (o, s) = model.run_fp16(&x, ExecutionTarget::NpuSingle, Some(&mut backend))?;
            (o, s, None)
        };
        if out.dims() != [1, 10] {
        ...
        if let Some(trace) = trace {
            ...
            fs::write("artifacts/mnist8-npu-trace.tsv", meta)?;
            fs::write("artifacts/mnist8-npu-trace-f16.bin", raw)?;
            trace_saved = true;
        }
    }
    if pred_match != 100 {
    ...
    fs::write("artifacts/mnist8-npu100-f16.bin", all_output_bits)?;
    let mean_abs = sum_abs as f32 / 1000.0;
    let stats = accepted_stats.unwrap();
    println!(
        "MNIST-8 CNN PASS nodes={} conv_cpu={} pool_cpu={} reshape_cpu={} add_cpu={} relu_cpu={} matmul_npu={} padded_npu={} top1_ref={}/100 ref_accuracy={}/100 npu_accuracy={}/100 max_abs={:.8} mean_abs={:.8} trace_saved={}",
        model.node_count(),
        stats.conv_nodes,
        stats.maxpool_nodes,
        stats.reshape_nodes,
