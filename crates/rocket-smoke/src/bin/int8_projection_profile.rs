...
fn bench(k: usize, n: usize, split: Int8DecodeSplit, workers: usize) -> Result<(), DynError> {
    ...
    // Alternate signs and zeros to expose stale input/output in reused scratch.
    // Repeating one input alone cannot detect a missing overwrite or old result.
    let activations: [Arc<[i8]>; 3] = [
        Arc::clone(&a),
        Arc::from(a.iter().map(|&x| -x).collect::<Vec<_>>()),
        Arc::from(vec![0i8; k]),
    ];
    let references = [
        reference.clone(),
        reference.iter().map(|&x| -x).collect::<Vec<_>>(),
        vec![0i32; n],
    ];
    ...
    for rep in 0..WARMUPS {
        let variant = rep % activations.len();
        let out = pool.execute_prepared(Arc::clone(&activations[variant]), &prepared)?;
        verify(&out.values, &references[variant])?;
    }
