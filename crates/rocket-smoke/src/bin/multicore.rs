...
fn run_workers(workers: usize) -> Result<(f64, f64, Vec<f64>), Box<dyn std::error::Error>> {
    ...
    for h in handles {
        match h.join() {
            ...
            Err(_) => return Err("multicore worker panicked".into()),
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for workers in order {
        ...
        let line = format!(
            "sample workers={workers} wall_ms={:.3} aggregate_GFLOPs={:.2} worker_seconds={:?}",
            wall * 1000.0,
            gflops,
            elapsed
        );
    ...
    let summary = format!(
        "median shape=M{M} K{K} N{N} reps_per_worker={REPS} GFLOPs[w1,w2,w3]=[{g1:.2},{g2:.2},{g3:.2}] scaling[w2,w3]=[{:.2}x,{:.2}x]",
        g2 / g1,
        g3 / g1
    );
    println!("{summary}");
    raw_lines.push(summary.clone());
    if g2 / g1 < 1.5 || g3 / g1 < 2.2 {
        return Err(format!("multicore scaling below conservative gate: {summary}").into());
    }
    println!("PASS: preconditioned separate-fd multicore RK3588 scaling probe; {summary}");
    fs::write(
        artifacts().join("multicore-probe.txt"),
        raw_lines.join("\n") + "\n",
    )?;
    Ok(())
}
