...
The cache contains all 154 (`22 x 7`) transformer-block projection weights. The three final-layer FFN projections first appear as M=1 during output-pruned prefill; the first autoregressive step finishes populating the cache, and the next step reuses it. Each distinct `(K,N)` decode geometry is tuned once across effective 1/2/3-worker N-splits using warmed, forward/reverse-interleaved median samples. The selector requires at least a 5% measured improvement before accepting more workers, then caches the chosen count and releases losing resident candidates. The final real-model run tuned four shapes and used 221 two-worker plus 90 three-worker calls. Its cache-hit average was `1.485 ms`, down from `2.573 ms` on the previous single-fd resident path (`1.73x`, about 42% less latency). The model output head has `N=32000` and remains on CPU because RockNPU deliberately caps the current W8A8 path at `N<=8192`. Batching, permutations, F16 M=1, and additional GGML ops remain unsupported; further work is now kernel/submit efficiency rather than repeated weight packing or a fixed worker table.
